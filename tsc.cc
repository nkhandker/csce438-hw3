...
class Client : public IClient
{
    ...
    private:
		...
		std::unique_ptr<SNSService::Stub> r_stub_;
...
int Client::connectTo()
{
	...
	// ------------------------------------------------------------
    std::string router_info = hostname + ":" + port;
    std::cout << router_info << std::endl;
    r_stub_ = std::unique_ptr<SNSService::Stub>(SNSService::NewStub(
               grpc::CreateChannel(
                    router_info, grpc::InsecureChannelCredentials())));
  ...
  if(stat.ok()){
	...
	server_port = ip_info.port();
	std::string login_info = servername + ":" + server_port;
    ...
    if(!ire.grpc_status.ok()) {
		...
		Status stat1 = r_stub_->Report_Comm_Err(&my_context1, ip_info, &pong);
		if (pong.code() == 2)
			return connectTo();
		if (pong.code() == 0)
			return -1;
...
void Client::comm_err()
{
	ClientContext context;
	Ip_info ip_info;
	ip_info.set_ip(servername);
	ip_info.set_port(server_port);
	Ping pong;
	Status stat1 = r_stub_->Report_Comm_Err(&context, ip_info, &pong);
	if (pong.code() == 2) {
		int ret = connectTo();
	}
	if (pong.code() > 0)
		std::cout << "Try command again" << std::endl;
}
...
IReply Client::Follow(const std::string& username2) {
    ...
    Status status = stub_->Follow_plus(&context, request, &reply);
...
IReply Client::UnFollow(const std::string& username2) {
    ...
    Status status = stub_->UnFollow_plus(&context, request, &reply);
    IReply ire;
    ire.grpc_status = status;
